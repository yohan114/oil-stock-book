...
export default function TransactionForm({ onClose, onSaved, defaultProductId, defaultKind = 'issue' }) {
  ...
  const [target, setTarget] = useState('machine'); // machine | project | other
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (kind === 'issue') {
        if (target === 'machine' && asset) { payload.asset_id = asset.id; payload.description = asset.ec_code || asset.registration; }
        else if (target === 'project' && projectId) { payload.project_id = Number(projectId); payload.description = projects.find((p) => p.id === Number(projectId))?.name; }
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="number" step="0.01" min="0" className="input" value={amount}
          ...
          {product && (
            <div className="flex items-end">
              <p className="text-xs text-slate-400">
                ...
                {kind === 'issue' && amount > 0 && (
                  <> → <b className={product.balance - amount < 0 ? 'text-rose-600' : 'text-emerald-600'}>{qty(product.balance - Number(amount), product.unit)}</b></>
                )}
              </p>
            </div>
          )}
        ...
        {kind === 'issue' ? (
          <div className="space-y-3 border-t border-slate-100 pt-3">
            ...
            {target === 'project' && (
              <div>
                <label className="label">Issued to project / site</label>
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button type="submit" className="btn-primary" disabled={busy}>{busy ? 'Saving…' : 'Save movement'}</button>
        </div>
      </form>
    </Modal>
  );
